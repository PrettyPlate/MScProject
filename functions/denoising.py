...
def compute_alpha(beta, t):
    ...
    alpha = (1 - beta).cumprod(dim=0)
    a = alpha.index_select(0, t + 1).view(-1, 1, 1, 1)
    return a
...
def abduction_steps(x_0, seq, model, b, y=None, **kwargs):
    assert kwargs.get("eta", 0) == 0
    with torch.no_grad():
        n = x_0.size(0)
        seq_next = list(seq[1:]) + [len(b)-1]
        # x0_preds = []
        xs = [x_0]
...
def ddpm_steps(x, seq, model, b, y=None, **kwargs):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        xs = [x]
